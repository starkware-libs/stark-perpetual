...
const { assertInRange, assertInMultiRange, Range } = require('./message_utils.js');
...
const vaultIDAllowedRanges = [
    new Range(zeroBn, twoPow31Bn), new Range(twoPow63Bn, twoPow63Bn.add(twoPow31Bn))
];
...
function hashTransferMsgWithFee(
    ...
    const tmpHash = pedersen([
        pedersen([transferToken, feeToken]), receiverPublicKey
    ]);
    if (condition === null) {
        msgHash = pedersen([
        ...
        msgHash = pedersen([
            pedersen([
                pedersen([tmpHash, condition]), packedMessage1.toString(16)
            ]), packedMessage2.toString(16)
        ]);
    }
...
function hashLimitOrderMsgWithFee(
    ...
    let msgHash = null;
    const tmpHash = pedersen([
        pedersen([tokenSell, tokenBuy]), feeToken
    ]);
...
function getTransferMsgHashWithFee(
    ...
    assert(
        hasHexPrefix(feeToken) && hasHexPrefix(token) &&
        hasHexPrefix(receiverStarkKey) && (condition === null ||
        hasHexPrefix(condition)), 'Hex strings expected to be prefixed with 0x.'
    );
    const amountBn = new BN(amount, 10);
    const nonceBn = new BN(nonce);
...
module.exports = {
    ...
    Range,  // Class.
    pedersen, getLimitOrderMsgHash, getTransferMsgHash, sign, verify, assertInRange,
    assertInMultiRange, getTransferMsgHashWithFee, getLimitOrderMsgHashWithFee // Function.
};
