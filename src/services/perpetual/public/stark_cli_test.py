...
def test_cli_hash_args(data_file, cli_file):
    command = ' '.join(
        [
            sys.executable, cli_file, '--method', 'hash', '--oracle', '4d616b6572', '--asset',
            '42544355534400000000000000000000',
            '--price', '000000000000000000000000000000000000000000000000ac9f3163ad52b000',
            '--time', '000000000000000000000000000000000000000000000000000000005f590c1e'])
    cli_run = subprocess.run(command, shell=True, capture_output=True)

    assert cli_run.stderr == b''
    assert bytes(hex(
        pedersen_hash(0x425443555344000000000000000000004d616b6572, 0xac9f3163ad52b0005f590c1e)
    )[2:] + '\n', 'utf-8') == cli_run.stdout


illegal_tests_list = [
    (
        '14d616b6572', '42544355534400000000000000000000',
    ...
    (
        '4d616b6572', '42544355534400000000000000000000',
        '000000000000000000000000000000000000000000000000ac9f3163ad52b000',
        '000000000000000000000000000000000000000000000000000000015f590c1e'),
    (
        '4d616b6572', '4254435553440000000000000000000000',
        '000000000000000000000000000000000000000000000000ac9f3163ad52b000',
        '000000000000000000000000000000000000000000000000000000005f590c1e'),
    (
        '4d616b6572', '42544355534400000000000000000000',
...
def test_cli_hash_illegal_params(data_file, cli_file, oracle, asset, price, time):
    command = ' '.join(
        [
            sys.executable, cli_file, '--method', 'hash', '--oracle', oracle,
            '--asset', asset, '--price', price, '--time', time])
    cli_run = subprocess.run(command, shell=True, capture_output=True)
...
def test_public_key(key_file, cli_file):
    ...
    command = ' '.join(
        [sys.executable, cli_file, '--method', 'get_public', '--key', private])
    cli_run = subprocess.run(command, shell=True, capture_output=True)
    assert b'' == cli_run.stderr
    assert bytes(public + '\n', 'utf-8') == cli_run.stdout
