...
def perpetual_messages_file() -> Dict[str, dict]:
    json_file = os.path.join(os.path.dirname(__file__), "perpetual_messages_precomputed.json")
...
def test_withdrawal_to_address_precomputed(perpetual_messages_file: Dict[str, dict]):
    # Tests that the get_withdrawal_to_address_msg function produces hashed messages as expected.
    items = perpetual_messages_file["withdrawal_to_address"].items()
    for expectedMessageHash, messageData in items:
        messageHash = get_withdrawal_to_address_msg(
            asset_id_collateral=messageData["assetIdCollateral"],
            eth_address=messageData["ethAddress"],
            position_id=messageData["positionId"],
            nonce=messageData["nonce"],
            expiration_timestamp=messageData["expirationTimestamp"],
