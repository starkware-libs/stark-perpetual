...
struct Deleverage:
    member deleveragable_position_id : felt
    member deleverager_position_id : felt
    member synthetic_asset_id : felt
    member amount_synthetic : felt
    member amount_collateral : felt
    member deleverager_is_buying_synthetic : felt
end
...
func execute_deleverage(
    ...
    let (range_check_ptr, positions_dict : DictAccess*, local deleveragable_funded_position,
        local deleveragable_updated_position, return_code) = update_position_in_dict(
    ...
    let (range_check_ptr, local positions_dict, deleverager_funded_position : Position*, _,
        return_code) = update_position_in_dict(
        range_check_ptr=range_check_ptr,
    ...
    assert_le_250_bit{range_check_ptr=range_check_ptr}(
        %[2**224%] + (updated_tv - FXP_32_ONE) * initial_tr + 1,
        %[2**224%] + initial_tv * updated_tr)
    %{ del error_code %}
