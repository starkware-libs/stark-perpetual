...
from starkware.cairo.common.math import (
    assert_in_range, assert_le_250_bit, assert_nn_le, assert_not_equal)

struct Liquidate:
    member liquidator_order : LimitOrder*
    # liquidator_position_id = liquidator_order.position_id.
    member liquidated_position_id : felt
    member actual_collateral : felt
    member actual_synthetic : felt
    member actual_liquidator_fee : felt
end
...
func execute_liquidate(
    ...
    let (range_check_ptr, local liquidated_funded_position) = position_apply_funding(
    ...
    assert_le_250_bit{range_check_ptr=range_check_ptr}(updated_tv * FXP_32_ONE + 1, updated_tr)
    %{ del error_code %}

    # We need to check that the synthetic balance in the liquidated position won't grow and will
    # keep the same sign.
    let (range_check_ptr, local initial_liquidated_asset_balance) = position_get_asset_balance(
        range_check_ptr=range_check_ptr,
        position=liquidated_funded_position,
