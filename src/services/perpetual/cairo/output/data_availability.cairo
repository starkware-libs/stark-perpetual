...
func output_availability_data(
        range_check_ptr, output_ptr : felt*, squashed_state : SquashedCarriedState*,
        perpetual_outputs_start : PerpetualOutputs*, perpetual_outputs_end : PerpetualOutputs*) -> (
        range_check_ptr, output_ptr : felt*):
    alloc_locals
    # Serialize the funding indices table.
    let (callback_adddress) = get_label_location(label_value=funding_indices_info_ptr_serialize)
