...
func pack_third_party_indices_inner(src: felt*, len, input) -> (packed_message: felt) {
    if (len == 0) {
        return (packed_message=input);
    }

    let msg = input * MULTI_ASSET_ORDER_LIST_FIELD_SIZE_UPPER_BOUND + src[0];
    let (packed_msg) = pack_third_party_indices_inner(src=src + 1, len=len - 1, input=msg);
    return (packed_message=packed_msg);
}

// Used to pack a single felt with up to 20 indices.
func pack_third_party_indices(src: felt*, len) -> (packed_message: felt) {
    let (packed_msg) = pack_third_party_indices_inner(src=src, len=len, input=0);
    return (packed_message=packed_msg);
}
...
func append_third_party_indices{range_check_ptr, output_array: felt*}(
    ...
    let (packed_indices) = pack_third_party_indices(src=third_party_indices, len=pack_len);
